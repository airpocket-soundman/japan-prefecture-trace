# 県の形 (Japan Prefecture Trace) v1.0.1
...
| 操作 | キーボード | ボタン／ジェスチャ |
|------|-----------|--------|
| 線を描く（ペンON時） | マウスドラッグ／タッチ | - |
| ペンON/OFF切替 | `P` | ペン ON/OFF |
| 拡大／縮小 | `+` / `-` | ＋ ／ −、ホイール、ピンチ（ペンOFF時） |
| 表示リセット | `0` | ⟲ |
| パン（移動） | - | マウスドラッグ／指1本（ペンOFF時） |
| 直前の線を取り消し | `Z` または `Backspace` | 取り消し |
| すべて消す | `C` | クリア |
| 判定 | `Enter` | 判定する |
| 次の問題 | `Enter`（判定後）| 次の問題 |

ペンが**ON**のとき1本指／マウスドラッグは描画。ペンが**OFF**のとき同じ操作はパンに切り替わります。ピンチイン／アウトはペンOFF時に2本指で。マウスホイールはペン状態に関わらず常時拡縮します。

## 採点方法

ユーザーの描画と正解の県境を双方向にサンプリングし、平均最近傍距離をコンテンツ座標のピクセル単位で算出（投影後の元画素値、ズームに依存しない）。閾値18pxを上限に `(1 - ratio)^1.3` のガンマ補正で0〜100点に変換します。誤差が小さいほどスコアが急上昇するので、トレースの精度がはっきり点数に出ます。地図を拡大して丁寧になぞるほど誤差が小さくなる仕組みです。
